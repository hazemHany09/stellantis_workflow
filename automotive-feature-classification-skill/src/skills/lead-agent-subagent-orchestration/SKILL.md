---
name: lead-agent-subagent-orchestration
description: Lead-agent sub-skill for the classification phase. Partitions parameters across subagents, formulates contracts, spawns up to 3 concurrent subagents, monitors their working files, and performs serial consolidation as each completes — promoting per-parameter records into the category STATE and moving the working file to the Archive. Not loaded by subagents.
loader: lead
---
...
* Frozen `runs/<run-id>/params.csv`.
* `kb_dataset_id`.
* `car_identity` + `resolved_trim`.
...
For each partition, construct a contract matching [`../../references/lead-to-subagent-contract.md`](../../references/lead-to-subagent-contract.md) and validate against [`../../templates/subagent-contract.json.tmpl`](../../templates/subagent-contract.json.tmpl).

1. Pre-create the working file at `.harness/SubAgent/<agent-name>.md` using [`../../templates/subagent-working-file.md.tmpl`](../../templates/subagent-working-file.md.tmpl).
...
2. Validate each record against [`../../templates/intermediate-parameter-record.json.tmpl`](../../templates/intermediate-parameter-record.json.tmpl) and each traceability block against [`../../templates/traceability-block.json.tmpl`](../../templates/traceability-block.json.tmpl).
3. Validate every enum value against [`../../references/enums.md`](../../references/enums.md).
...
ARCH-2, ARCH-4, AS-HARN-D, AS-HARN-E, Q-HARN-13, AS-DEL-D, AS-DEL-F.
