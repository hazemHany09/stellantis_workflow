...
> **Scope:** This skill is loaded exclusively by the lead agent. The lead agent uses the template below directly in its own inline Python script to generate all contract files — no subagent is spawned for this step.

## Contract File

Each parameter gets one contract file in `classification-tasks/`. The lead agent uses the template below as the basis for an inline Python script (Stage 5) that writes all contract files in one pass, with the `## Task` section fully pre-populated.

### File naming
```
classification-tasks/<parameter_id>-<optimised_parameter_name_slug>.md
```
Example: `classification-tasks/P042-wireless-charging.md`

`parameter_id` is generated by the Stage 5 script as `P{row_index:03d}` (1-indexed). There is no ID column in `params-frozen.csv`.

### CSV column → contract field mapping

| CSV column | Contract field |
|------------|---------------|
...
| `Basic Criterion` | Level 1 row (omit if blank) |
...
*(Only levels defined in the reference list are included. If a level has no criterion, its row is omitted. Classify only to one of the levels present in this table — never assign a level not listed here.)*
