...
| Confidence          | 1           | One of `consensus`, `single-source`, `vague-only`, `silent-all`. Set deterministically by the rule engine — see the *Source-type categories and consensus* section in `10-decision-rules.md`. `consensus` is reserved for verdicts backed by ≥ 2 clear sources from distinct source-type categories.                                          |
| Inverse retrieval attempted | 1   | Boolean. Required `true` on every Rule 4 record (silent-all is admissible only after an inverse-language test of the same source set). May be `true` on other rules if such a pass also ran.                                                                                                                                                  |
| Decision rule       | 1           | One of `Rule-1`, `Rule-2a`, `Rule-2b`, `Rule-3`, `Rule-4`, `Rule-5`. The single rule that produced the record.                                                                                                                                                                                                                                |
...
| Source type                  | One of the seven categories listed in `10-decision-rules.md` (e.g. `manufacturer_official_spec`, `dealer_fleet_guide`, `forum_or_ugc`). Determines whether the block contributes to consensus or counts as the same category as another block on the same record.                                                                  |
| Stance                       | For Rule 2b blocks: `present` or `absent`. For inverse-retrieval evidence that promoted a parameter from Rule 4 to Rule 5: `absent`. Optional otherwise.                                                                                                                                                                            |
