...
| :- | :------------------------------- | :------------------------------------------- | :------------ | :------------------- | :----------- | :---------- |
| W1 | Normal pipeline                  | Client submits request (URLs optional)       | Yes           | Full car, all params | Yes          | Yes         |
| W2 | Automated mode (no gate)         | Client flags run as "automated"              | **No**        | Full car, all params | Yes          | Yes         |
| W3 | Full-car re-run                  | Client requests full redo                    | Yes           | Full car, all params | Yes          | Yes         |
| W4 | Gap-fill cycle (coverage-driven) | Client triggers after review                 | Optional      | Targeted params only | **Reuse**    | No (same)   |
| W5 | Per-parameter flagging re-run    | Client marks specific params                 | Optional      | Flagged params only  | Reuse or new | Depends     |
| W6 | Concurrent runs on same car      | Client initiates 2+ independent W1           | Yes (each)    | Full car × N         | Yes (each)   | Yes (each)  |
| W7 | Multi-car batch                  | Client submits N cars                        | Yes (each)    | One car per sub-run  | Yes (each)   | Yes (each)  |
| W8 | Input clarification sub-workflow | Triggered inline when required field missing | —             | Pre-pipeline         | —            | —           |

***

## W1 — Normal pipeline

**Preconditions:** client provides a brand, model, year, and market. URLs and/or domains are optional.

**Stages:**

1. Parse request, extract four required fields (prompt via W8 if any missing).
...
4. Build URL candidate set:
   * **If the client supplied at least one URL or domain** — accept them, canonicalise + dedupe + paywall-filter. No cap. Tag each URL `origin = client`.
   * **If the client supplied no URLs or domains** — perform a web search using `brand + model + year + market` as query context, canonicalise + dedupe + paywall-filter, and cap the list at 20 URLs (or a client-specified cap).
...
**Notes:** If zero candidates survive the paywall-filter (in either sub-mode), the agent proceeds with zero URLs and W1 collapses into Rule-4 everywhere (every parameter: silent-all).

***

## W2 — Automated mode (no approval gate)

**Preconditions:** client explicitly flags the run as "automated / no-gate". Same required inputs as W1.

**Stages:**

1. Parse / resolve / load CSV (same as W1 steps 1–3).
2. Build URL candidate set (client-supplied if provided; agent-discovered otherwise).
...
## W4 — Gap-fill cycle (coverage-driven)
...
## W5 — Per-parameter flagging re-run
...
## W6 — Concurrent runs on the same car
...
**Behaviour:** the entry-point workflow decomposes the batch into N independent single-car invocations (W1 / W2 depending on whether the client also flags automated mode). The batch workflow itself is just a fan-out — each sub-run is a full W1/W2 run with its own approval gate, KB, and deliverable.
...
## W8 — Input clarification sub-workflow
...
2. Client responds. Agent resumes the outer workflow (W1 / W2) from step 2 (car-identity resolution).
...
**Notes:** W8 is the only workflow that can be invoked mid-run. It is a pre-pipeline interrupt, not a parallel branch.
...
1. **W1 and W2 are mutually exclusive per run** — a run is gated (W1) or automated (W2), never both.
2. **W3 is a new invocation of W1 / W2** — "re-run" is a client-facing label, not an architectural distinction.
3. **W4 and W5 only attach to a completed normal run** — they cannot be triggered in isolation; they need an existing deliverable + KB to extend.
4. **W4 and W5 can be chained** — a client may run W4 to add sources, then W5 to re-classify on the enlarged KB.
5. **W6 and W7 are orchestration patterns around W1 / W2** — they do not add new behaviour, only concurrency shape.
6. **W8 is invoked inline at most once per run** — once all four fields are present, W8 is not re-entered.
7. **Gap-fill cycles within a run do not change the run ID.** Full-car re-runs do. This is the single most important distinction between W3 and W4.
...
flowchart TD
    ...
    BATCH -- yes --> W7["W7 — Multi-car batch<br/>(fan out to W1 / W2)"]
    BATCH -- no --> GATE{"Gated run?"}
    GATE -- no --> W2["W2 — Automated, no gate"]
    GATE -- yes --> W1["W1 — Normal pipeline<br/>(URLs optional)"]



    EXIST -- yes --> INTENT{"Client intent?"}
    INTENT -- redo from scratch --> W3["W3 — Full-car re-run"]
    INTENT -- more evidence for<br/>silent / weak params --> W4["W4 — Gap-fill cycle"]
    INTENT -- re-classify specific<br/>params on existing KB --> W5["W5 — Per-parameter flag"]

    START --> PARALLEL{"Other run<br/>already active on<br/>same car?"}
    PARALLEL -- yes --> W6["W6 — Concurrent runs<br/>(each fully independent)"]

    START --> MISS{"Any of 4 required<br/>fields missing?"}
    MISS -- yes --> W8["W8 — Input clarification"]
    W8 --> GATE
```
