...
| `confidence`          | `consensus` \| `single-source` \| `vague-only` \| `silent-all` | Always | Strength of agreement behind the verdict. Set deterministically — see *Source-type categories and consensus* in `10-decision-rules.md`. `consensus` requires ≥ 2 clear sources from distinct source-type categories.                                                                                                                                              |
| `inverse_retrieval_attempted` | boolean                                       | Always (`true` required on Rule 4)                     | True iff an inverse-language test of the source set was performed. Mandatory on Rule 4 records (silent-all may not be finalised without it). Optional on other rules.                                                                                                                                                                                       |
| `decision_rule`       | `Rule-1` \| `Rule-2a` \| `Rule-2b` \| `Rule-3` \| `Rule-4` \| `Rule-5` | Always | The single rule that produced the record. Redundant with `(presence, status, classification)` but emitted explicitly for audit and easier human review.                                                                                                                                                                                                       |
| `traceability_blocks` | list of traceability blocks (see `09-deliverable.md`) | At least one when `presence = Yes`; optional otherwise | Each block carries source name, source link, source type, optional stance, and its own justification text. Multiple blocks may support a single parameter verdict. See `09-deliverable.md`.                                                                                                                                                                  |
...
11. `confidence` is always set. `consensus` is reserved for Rule 1 / Rule 5 records backed by ≥ 2 clear sources from distinct source-type categories. `single-source` is the only other legal value for Rule 1 / Rule 5. `vague-only` is exclusive to Rule 3. `silent-all` is exclusive to Rule 4.
12. Every Rule 4 record carries `inverse_retrieval_attempted = true`. A Rule 4 record without this flag is invalid and must not be emitted.
13. No Rule 1 / Rule 5 record may rest on `forum_or_ugc` as its only clear source — such verdicts are demoted to Rule 3 by the rule engine before emission.
