...
- **Gap-fill KB reuse** (AS-KB-C / Q-KB-6): the gap-fill workflow extends the original KB rather than creating a fresh one, so citation identity is continuous across the normal run and the gap-fill.
