...
**Assumption.** When `(brand, model, year, market)` maps to multiple variants (body styles, facelifts, regional rebadges, sub-models), we **auto-select the most prominent / widespread variant** and record the resolved identity in the deliverable header. You can request a re-run with a different variant if needed.

**Why we ask.** This is the single biggest silent-behaviour decision in the skill. An auto-resolution that picks the "wrong" variant will produce a technically correct but clinically wrong deliverable.

**Options.**

* (A) Confirm auto-select, surface choice in header. *(current assumption)*
* (B) Always pause and ask you to pick among candidates before we proceed.
* (C) Require you to always include a disambiguator (body style, sub-model code) in the submission itself.

**Risk if wrong.** You accept a deliverable built on the wrong variant without noticing the header line that records the choice.
...
### Q-C-14 — Silent-all → `Presence = No, Status = Success` — acceptable?

**Assumption.** When every reviewed source is silent on a parameter, we treat it as "not found in reviewed sources" and emit `Presence = No, Status = Success, Classification = Empty`. The distinction between "no evidence at all" (Rule 4) and "evidence explicitly said absent" (Rule 5) is visible only in the traceability block set (Rule 5 has blocks; Rule 4 has zero).

**Why we ask.** This collapses the former `No Information Found` into `No`. If you rely on the "no information" label to flag parameters for follow-up research, that signal is now only discoverable by counting traceability blocks.

**Options.**

* (A) Confirm silent-all = `No` / `Success`. *(current assumption)*
* (B) Re-introduce `No Information Found` as a distinct enum value for silent-all, separate from `No`.
* (C) Keep silent-all = `No` but add an explicit boolean field `evidence_based: true/false` on each record.

**Risk if wrong.** With (A), analysts cannot distinguish "car does not have this feature" from "our sources did not discuss this feature" without inspecting the JSON structure.
